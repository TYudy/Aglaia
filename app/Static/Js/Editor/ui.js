...
function updatePanel(selection) {
  if (!selection) {
    ...
    $('#preset').append("<option value='custom'>Personalizado</option>");
...
function populateGrid(type) {
  if (type == 'shape-tool') {
    ...
    if (!flag) {
      $('#upload-tabs').after(
        '<div id="landing" class="upload-landing"><div id="landing-text">Las imágenes cargadas aparecerán aquí para facilitar el acceso.</div></div>'
      );
    }
    ...
    if (!flag) {
      $('#upload-tabs').after(
        '<div id="landing" class="upload-landing"><div id="landing-text">Los videos subidos aparecerán aquí para facilitar el acceso.</div></div>'
      );
    }
    ...
    $('#shapes-cont').append("<p class='row-title'>Animado</p>");
    $('#shapes-cont').append(
...
function search() {
  ...
  if ($('#image-tool').hasClass('tool-active')) {
        ...
        if (parseInt(data.totalHits) > 0) {
          ...
          $('#shapes-cont').html(
            "<div id='no-results'>Sorry, we couldn't find any results for &#x22;" +
              encodeURIComponent(value) +
              '&#x22;. Por favor intente con una consulta diferente.</div>'
          );
        }
      });
    } else {
    ...
    if (value == '') {
      $('#shapes-cont').html(
        '<p class="row-title">Formas</p><div class="gallery-row" id="shapes-row"></div><p class="row-title">Emojis</p><div class="gallery-row" id="emojis-row"></div>'
      );
